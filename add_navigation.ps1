...
foreach ($file in $htmlFiles) {
    ...
    # 检查是否已经添加了导航功能
    if ($content -notcontains "navigation") {
        # 添加CSS样式
        $content = $content -replace "(#theater-view \{[^}]+\})", "$1
        ...
        # 添加导航按钮HTML
        $content = $content -replace "(<div id=\"main-stage\">)", "<!-- 导航栏 -->
    ...
    $1"
        ...
        # 添加导航功能脚本
        $content = $content -replace "(</script>\s*</body>)\s*$", "$1
